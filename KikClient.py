...
class KikClient():
    ...
    def get_info_for_jid(self, node):
        jid = node + "@talk.kik.com"
    ...
    def send_message(self, node, body):
        print("[+] Sending message \""+body+"\" to "+node+"...")
        if "@" in node:
            jid = node
        else:
            jid = node + "@talk.kik.com"
    ...
    def send_is_typing(self, node, is_typing):
    ...
    def add_friend(self, node):
        ...
        jid_info = dict()
        jid_info["node"] = node[node.find("@")] if "@" in node else node
        jid_info["display_name"] = Utilities.extract_tag_from_xml(response, "display-name")
        jid_info["username"] = Utilities.extract_tag_from_xml(response, "username")
        print("[+] Okay")
        return jid_info

    def send_read_confirmation(self, node, message_id):
