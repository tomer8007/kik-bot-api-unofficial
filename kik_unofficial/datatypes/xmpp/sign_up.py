...
class RegisterRequest(XMPPElement):
    ...
    def __init__(self, email, username, password, first_name, last_name, birthday, captcha_result=None, device_id=None, android_id=None):
        super().__init__()
        self.email = email
...
class CheckUsernameUniquenessRequest(XMPPElement):
    ...
    def serialize(self) -> bytes:
        data = f'<iq type="get" id="{self.message_id}">' '<query xmlns="kik:iq:check-unique">' f"<username>{self.username}</username>" "</query>" "</iq>"
